...
\fancyhead[R]{\small Unpaired Diffusion (Pelvis)}
...
{\normalsize\bfseries Model: Unpaired Diffusion (Pelvis)}\\[4pt]
\small
...
\caption{Dual-Path Latent Diffusion: Pelvis modality translation is achieved by conditioning the denoising process on the encoded latent of the source domain, with cycle-consistency enforced to ensure anatomical invariance.}
\end{figure}

\textbf{Anatomical Preservation via Latent Cycle-Consistency:} 
The implementation enforces structural fidelity by requiring that a CT latent encoded through the \texttt{a2b} path and translated back via the \texttt{b2a} path remains close to the original latent. This latent-space cycle-consistency is specifically beneficial for pelvic imaging, where the diverse density range of bone and soft tissue requires a stable, reversible mapping to avoid hallucinating structures.

\textbf{VAE Skip-Injection and LoRA Adapters:} 
The unpaired pelvis implementation utilizes custom VAE skip connections that project intermediate encoder features into the decoder via $1 \times 1$ convolutions. This preserves sharp boundaries of the pelvic girdle. Both the UNet and VAE are fine-tuned using Low-Rank Adaptation (LoRA), allowing for parameter-efficient adaptation of the generative model to the SynthRAD pelvic modality.
