# Unpaired Diffusion (Pelvis) Documentation

## Basic Information
- **Model Name**: Unpaired Diffusion (Pelvis)
- **Pipeline Path**: `project-group-5/models/unpaired_diffusion/pelvis`
- **Architecture Type**: Bidirectional Cycle-Consistent Latent Diffusion Model
- **Region**: Pelvis
- **Modality**: Unpaired (CT ↔ MRI)
- **Purpose**: Map pelvis CT scans to MRI scans (and vice versa) using a pre-trained single-step diffusion model (SD-Turbo), trained on unpaired datasets via cycle-consistency.
...
### Training Workflow
- The model takes independent, unpaired batches of CT and MRI slices of the pelvic region.
...
- Retains the fast inference of the 1-step SD-Turbo architecture.
- VAE skip-connections preserve highly critical anatomical edge structures in pelvic organs.
...
```mermaid
graph TD
    A[2.5D Pelvis CT Input] -->|A2B| B[VAE Encoder A2B]
    B --> C[Latent Space]
...
```mermaid
flowchart LR
    CT[Unpaired Pelvis CT Data] --> SlicerA[Extract 3 CT Slices]
    MRI[Unpaired Pelvis MRI Data] --> SlicerB[Extract 3 MRI Slices]
    SlicerA --> T[Resizing & Padding]
    SlicerB --> T
    T --> N[Norm to -1,1]
    N --> Model[Bidirectional Forward Passes]
    Model --> OutA[Fake Pelvis MRI]
    Model --> OutB[Fake Pelvis CT]
```
...
```mermaid
graph TD
    Input[New Pelvis CT Patient Data] --> Preproc[2.5D Extraction & Norm]
