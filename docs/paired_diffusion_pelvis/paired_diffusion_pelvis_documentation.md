# Paired Diffusion (Pelvis) Documentation

## Basic Information
- **Model Name**: Paired Diffusion (Pelvis)
- **Pipeline Path**: `project-group-5/models/paired_diffusion/pelvis`
- **Architecture Type**: Latent Diffusion Model (1-Step Translation) with LoRA and VAE Skip Connections
- **Region**: Pelvis
- **Modality**: Paired (CT to MRI)
- **Purpose**: Directly map pelvis CT scans to pelvis MRI scans using a pre-trained single-step diffusion model (SD-Turbo) fine-tuned on paired medical data.
...
### Training Workflow
- The model is trained using paired CT-MRI slices of the pelvic region. 
...
```mermaid
graph TD
    A[2.5D Pelvis CT Input] --> B[VAE Encoder]
    B --> C[Latent Space]
...
```mermaid
flowchart LR
    CT[Pelvis CT Volumetric Data] --> Slicer[Extract 3 Adjacent Slices]
    Slicer --> T[Resizing & Padding]
    T --> N[Min-Max Norm to -1,1]
    N --> Model[Paired Diffusion Forward Pass]
    Model --> Out[Generated Pelvis MRI 2.5D Slice]
```
