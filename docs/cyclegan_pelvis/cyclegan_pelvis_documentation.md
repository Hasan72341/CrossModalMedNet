# CycleGAN (Pelvis) Documentation

## Basic Information
- **Model Name**: CycleGAN (Pelvis)
- **Pipeline Path**: `project-group-5/models/cyclegan/pelvis`
- **Architecture Type**: Cycle-Consistent Generative Adversarial Network (CycleGAN)
- **Region**: Pelvis
- **Modality**: Unpaired (CT ↔ MRI)
- **Purpose**: Bidirectional, unsupervised translation between pelvis CT and MRI slices using cycle consistency to circumvent the need for perfectly aligned training pairs.
...
### Training Workflow
- The model trains using independent, unpaired sets of CT and MRI 2D slices of the pelvic region.
...
graph TD
    subgraph Generators
    A[2D Pelvis CT Input] --> G[ResNet Generator CT->MRI]
    G --> B[9x Residual Blocks]
    B --> C[Fake Pelvis MRI Output]
    
    A2[2D Pelvis MRI Input] --> G2[ResNet Generator MRI->CT]
    G2 --> B2[9x Residual Blocks]
    B2 --> C2[Fake Pelvis CT Output]
    end
    
    subgraph Discriminators
    C --> D1[PatchGAN D_MRI]
    RealMRI[Real Pelvis MRI] --> D1
    
    C2 --> D2[PatchGAN D_CT]
    RealCT[Real Pelvis CT] --> D2
    end
```
...
```mermaid
flowchart LR
    CT[Unpaired Pelvis CT] --> Slicer[Extract Random 2D CT]
    MRI[Unpaired Pelvis MRI] --> SlicerB[Extract Random 2D MRI]
    Slicer --> N1[Norm to -1,1]
    SlicerB --> N2[Norm to -1,1]
    N1 --> G[G: CT->MRI]
    N2 --> F[F: MRI->CT]
    G --> OutMRI[Fake Pelvis MRI]
    F --> OutCT[Fake Pelvis CT]
```
...
```mermaid
graph TD
    Input[New Pelvis CT Patient Volume] --> Slicer[Sequential 2D Slicing]
    Slicer --> Norm[Clamping & Norm]
    Norm --> Gen[Trained ResNet Generator: CT->MRI]
    Gen --> Out[2D Fake Pelvis MRI Slice]
