# Pix2Pix (Pelvis) Documentation

## Basic Information
- **Model Name**: Pix2Pix (Pelvis)
- **Pipeline Path**: `project-group-5/models/pix2pix/pelvis`
- **Architecture Type**: Conditional Generative Adversarial Network (cGAN)
- **Region**: Pelvis
- **Modality**: Paired (CT to MRI)
- **Purpose**: Direct, deterministic translation of 2D pelvis CT slices into corresponding 2D MRI slices using a paired U-Net Generator and PatchGAN Discriminator setup.
...
### Training Workflow
- The model trains using perfectly paired CT and MRI 2D slices of the pelvic region.
...
```mermaid
graph TD
    A[2D Pelvis CT Input] --> G[Unet Generator]
    G -->|Downsampling| B[Bottleneck]
    B -->|Upsampling + Skip Connections| C[2D Fake Pelvis MRI Output]
    
    C --> D[PatchGAN Discriminator]
    E[2D Real Pelvis MRI] --> D
...
```mermaid
flowchart LR
    CT[Pelvis CT Volumetric Data] --> Slicer[Extract Single 2D Slice]
    MRI[Pelvis MRI Volumetric Data] --> SlicerB[Extract Matching 2D Slice]
    Slicer --> N1[Clamp -1000,1000 & Norm to -1,1]
    SlicerB --> N2[Min-Max Norm to -1,1]
    N1 --> G[Generator Forward Pass]
    G --> Out[Fake 2D Pelvis MRI]
```
