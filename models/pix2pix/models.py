"""
pix2pix_standard/models.py
-------------------------
Neural network architecture for Pix2Pix CT → MRI translation.
Standard implementation with LSGAN (MSE) loss for bounded discriminator loss [0, 1].
"""
...
class Pix2Pix(nn.Module):
    ...
    def generator_loss(
        ...
        # 1. Adversarial loss (Discriminator sees CT + Fake MRI)
        pred_fake = self.D_MRI(torch.cat([ct, fake_mri], dim=1))
        # Note: LSGAN with target 1.0 is in [0, 1]
        loss_g_gan = self.criterion_gan(pred_fake, torch.ones_like(pred_fake))
        
        # 2. L1 loss
        loss_g_l1 = self.criterion_l1(fake_mri, real_mri)
        
        # 3. Identity loss (optional)
        loss_g_id = torch.tensor(0.0, device=ct.device)
        if self.lambda_identity > 0:
            loss_g_id = self.criterion_l1(self.G_CT2MRI(real_mri), real_mri)
    ...
    def _discriminator_loss(
        ...
        """
        # Real
        pred_real = D(torch.cat([ct, real_mri], dim=1))
        loss_d_real = self.criterion_gan(pred_real, torch.ones_like(pred_real))
        
        # Fake
        pred_fake = D(torch.cat([ct, fake_mri.detach()], dim=1))
