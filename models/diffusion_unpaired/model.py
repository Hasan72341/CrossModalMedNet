...
class VAEEncode(nn.Module):
    def __init__(self, vae: AutoencoderKL, vae_b2a: AutoencoderKL | None = None) -> None:
        super().__init__()
        self.vae = vae
    ...
    def forward(self, x: torch.Tensor, direction: str) -> torch.Tensor:
        ...
        active_vae = self.vae if direction == "a2b" else self.vae_b2a
        if active_vae is None:
            raise ValueError("VAE for direction is not initialized")
        return active_vae.encode(x).latent_dist.sample() * active_vae.config.scaling_factor
...
class VAEDecode(nn.Module):
    ...
    def forward(self, x: torch.Tensor, direction: str) -> torch.Tensor:
        ...
        if active_vae is None:
            raise ValueError("VAE for direction is not initialized")
        active_vae.decoder.incoming_skip_acts = active_vae.encoder.current_down_blocks
        decoded = active_vae.decode(x / active_vae.config.scaling_factor).sample
...
def forward_with_networks(
    ...
    x_out = torch.stack(
        [
            scheduler.step(model_pred[i], timesteps[i], x_enc[i], return_dict=True).prev_sample
...
def get_trainable_params(
    unet: UNet2DConditionModel,
    vae_a2b: AutoencoderKL,
    vae_b2a: AutoencoderKL,
) -> list[nn.Parameter]:
    params: list[nn.Parameter] = []
    seen_ids = set()

    def add_unique(new_params):
        for p in new_params:
    ...
    for name, param in vae_a2b.named_parameters():
        if "lora" in name and "vae_skip" in name:
            add_unique([param])
    add_unique(vae_a2b.decoder.skip_conv_1.parameters())
    add_unique(vae_a2b.decoder.skip_conv_2.parameters())
    add_unique(vae_a2b.decoder.skip_conv_3.parameters())
    add_unique(vae_a2b.decoder.skip_conv_4.parameters())

    for name, param in vae_b2a.named_parameters():
        if "lora" in name and "vae_skip" in name:
            add_unique([param])
    add_unique(vae_b2a.decoder.skip_conv_1.parameters())
    add_unique(vae_b2a.decoder.skip_conv_2.parameters())
    add_unique(vae_b2a.decoder.skip_conv_3.parameters())
    add_unique(vae_b2a.decoder.skip_conv_4.parameters())
...
def load_lora_checkpoint(
    ...
    vae: AutoencoderKL,
    device: torch.device,
) -> tuple[AutoencoderKL, VAEEncode, VAEDecode, dict]:
    ...
    # Map state dict keys. Older checkpoints might have separated SDs.
    # We unify them into the single 'default' adapter.
    unet_sd = {}
    if "sd_unet" in sd:
        unet_sd = sd["sd_unet"]
    else:
        # Backward compatibility for old checkpoints with 3 adapters
        for k, v in sd.get("sd_encoder", {}).items():
            unet_sd[k.replace(".default_encoder.", ".default.")] = v
        for k, v in sd.get("sd_decoder", {}).items():
            unet_sd[k.replace(".default_decoder.", ".default.")] = v
        for k, v in sd.get("sd_other", {}).items():
            unet_sd[k.replace(".default_others.", ".default.")] = v
    ...
    vae_enc = VAEEncode(vae, vae_b2a=vae_b2a)
    vae_enc.load_state_dict(sd["sd_vae_enc"])
    vae_dec = VAEDecode(vae, vae_b2a=vae_b2a)
    vae_dec.load_state_dict(sd["sd_vae_dec"])

    return vae_b2a, vae_enc, vae_dec, sd
