...
class Settings(BaseSettings):
    ...
    source_modality: Literal["mr", "ct"] = Field(default="ct", validation_alias="SOURCE_MODALITY")
    ...
    lambda_cycle: float = Field(default=1.0, validation_alias="LAMBDA_CYCLE")
    lambda_cycle_lpips: float = Field(default=10.0, validation_alias="LAMBDA_CYCLE_LPIPS")
    lambda_idt: float = Field(default=1.0, validation_alias="LAMBDA_IDT")
    lambda_idt_lpips: float = Field(default=1.0, validation_alias="LAMBDA_IDT_LPIPS")

    # Logging and outputs
    output_dir: str = Field(
        default=str(_MODULE_ROOT / "outputs"),
    ...
    full_eval_every_n_epochs: int = Field(default=5, validation_alias="FULL_EVAL_EVERY_N_EPOCHS")
